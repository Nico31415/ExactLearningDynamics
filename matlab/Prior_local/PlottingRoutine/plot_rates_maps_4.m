function []=plot_rates_maps_4(fig_num,name,name_fig,matrix1,matrix2,matrix3,matrix4,line,name_x,name_y)
% Plot and save 4 coulour map
    ...
    Maxi_3= max(matrix3,[],'all');
    Mini_3= min(matrix3,[],'all');
    Maxi_4= max(matrix4,[],'all');
    Mini_4= min(matrix4,[],'all');
    ...
    Vector_max_min_squared_3=[Maxi_3.^2,Mini_3.^2];
    l_3=sqrt(max(Vector_max_min_squared_3));
    Vector_max_min_squared_4=[Maxi_4.^2,Mini_4.^2];
    l_4=sqrt(max(Vector_max_min_squared_4));
    ...
    for i=line
        ...
        subplot(2,2,3)
        Plot_matrix(:,:)=matrix3(i,:,:);
        caxis('manual')
        colormap(cmap);
        imagesc(Plot_matrix);
        colorbar('Ticks', [-l_3, 0, +l_3], 'TickLabels',[-l_3,0,l_3],'FontSize',12)
        caxis([-l_3,+l_3])
        t1 = get(gca,'XTickLabel');
        t2 = get(gca,'YTickLabel');
        set(gca,'XTickLabel',[1,2,3,4,5,6,7,8],'FontName','Times','fontsize',12)
        ...
        subplot(2,2,4)
        Plot_matrix(:,:)=matrix4(i,:,:);
        caxis('manual')
        colormap(cmap);
        imagesc(Plot_matrix);
        colorbar('Ticks', [-l_4, 0, +l_4], 'TickLabels',[-l_4,0,l_4],'FontSize',12)
        caxis([-l_4,+l_4])
        t1 = get(gca,'XTickLabel');
        t2 = get(gca,'YTickLabel');
        set(gca,'XTickLabel',[1,2,3,4,5,6,7,8],'FontName','Times','fontsize',12)
