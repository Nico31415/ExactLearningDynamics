...
w1s, w2s, losses = model.train(X.T, Y.T, training_steps, learning_rate)
...
for color, output in zip(blind_colours, outputs.T):
    ...
    plt.plot((analytical @ X[:plot_items_n].T).reshape(training_steps, -1), lw=3., c="k", alpha=0.7, linestyle=(0, (1, 2)), label='analytical') # (0, (3, 4, 3, 1))
    
for color, target in zip(blind_colours, Y[:plot_items_n]):
    for value in target:
        plt.scatter(training_steps * 1.6, value, marker="_", color=color, lw=2.5)

plt.title('Learning Dynamics Theoretical vs Analytical')
