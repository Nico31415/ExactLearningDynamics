...
for n in range(num_trials):
    ...
    distance_timestep_pairs.append((dist, find_first_index(losses, threshold_loss, epsilon)))
...
starting_loss = ...
optimal_loss = ...
upper_bound3 = 1/mu * (np.log(starting_loss - optimal_loss) - np.log(epsilon))

# plt.plot(x_line, y_line, linestyle='--', color='red', label='upper bound 1')
# plt.plot(x_line, upper_bound2, linestyle='--', color='red', label='upper bound 2')
plt.legend()
plt.scatter(distances, timesteps, color ='blue')
