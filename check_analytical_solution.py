...
def check_analytical_solution(solution):
    ...
    if solution == '3':
        ...
        for i in range(1, training_steps):
            ...
            centre = np.linalg.inv(np.eye(lmda_inv.shape[0]) + 1/2 * Q0.T @ (O@e_lmdat@O.T@O@lmda_inv@O.T@e_lmdat@O.T - O@lmda_inv@O.T))
        ...
        for i in range(1, training_steps):
            ...
            centre = np.linalg.inv(np.eye(lmda_inv.shape[0]) + 1/2 * Q0.T @ (O@e_lmdat@lmda_inv@e_lmdat@O.T - O@lmda_inv@O.T)@Q0)
        ...
        for i in range(1, training_steps):
            ...
            centre = np.linalg.inv(np.eye(lmda_inv.shape) + 1/2 * Q0.T @ O @ (e_2lmdat @ lmda_inv - lmda_inv)@O.T@Q0)
        ...
        for i in range(1, training_steps):
            ...
            centre = np.linalg.inv(np.eye(...) + 1/2 * Q0.T @ O @ (e_2lmdat - np.eye(e_2lmdat.shape[0])@lmda_inv@O.T@Q0))
